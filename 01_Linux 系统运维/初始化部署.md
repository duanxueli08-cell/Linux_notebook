...
# 确认当前网卡 MAC 地址
ip link show ens33
# 创建 udev 规则文件（强制命名）（替换为你的 MAC）：
vi /etc/udev/rules.d/70-persistent-net.rules
SUBSYSTEM=="net", ACTION=="add", DRIVERS=="?*", ATTR{address}=="00:0c:29:11:07:c8", NAME="eth0"
```
- 这个文件名 70-persistent-net.rules 是约定俗成的，虽然旧版已弃用，但在现代 systemd 中仍有效（只要规则语法正确）
...
📌 补充说明
- ❓为什么不用 Netplan 直接改名？
	- Netplan 不能重命名接口，它只能配置已有接口。重命名必须在内核/udev 层完成。
